  ...
  return (
    <div className="mt-2 space-y-1.5">
      <div className="relative h-1.5 rounded-full bg-slate-100 overflow-hidden">
        <div className="absolute left-0 top-0 h-full rounded-full" style={{ width: `${unpackedPct}%`, background: "linear-gradient(90deg,#fbbf24,#f59e0b)" }} />
        <div className="absolute left-0 top-0 h-full rounded-full" style={{ width: `${packedPct}%`, background: "linear-gradient(90deg,#10b981,#059669)" }} />
        {ordered > 0 && !isOver && (
          <div className="absolute top-0 h-full w-0.5 bg-slate-600/60" style={{ left: `${Math.min((ordered / total) * 100, 99)}%` }} />
        )}
      </div>
  ...
  return (
    <div className="space-y-1.5">
      ...
      {mode === "dealer" && options.length > 0 && (
        <>
          <button ref={triggerRef} type="button" onClick={() => dropOpen ? setDropOpen(false) : openDrop()}
            ...
            <span className={`truncate font-semibold ${selectedDealer ? "text-slate-800" : "text-slate-400"}`}>
              {selectedDealer ? (selectedDealer.is_percentage ? `${selectedDealer.discount_value}% off` : `₹ ${selectedDealer.discount_value} off`) : "Pick discount…"}
            </span>
            <FiChevronDown size={11} className={`text-slate-400 flex-shrink-0 transition-transform ${dropOpen ? "rotate-180" : ""}`} />
          </button>
          {dropOpen && (
            <div ref={panelRef} style={panelStyle} className="bg-white border border-slate-200 rounded-xl shadow-[0_16px_48px_-8px_rgba(15,23,42,0.16)] overflow-hidden">
...
/* ================================================================
   ORDER SUMMARY CARD — Premium redesign
   ================================================================ */
const OrderSummaryCard = ({ financialSummary, itemCount }) => {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
      ...
      {/* Balance + savings banner */}
      <div className="px-5 pb-5 pt-1 space-y-2.5">
        {/* Balance */}
        <div className={`flex items-center justify-between px-4 py-3.5 rounded-xl border-2 ${isPaid
          ...
          <div className="flex items-center gap-2.5">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isPaid ? "bg-emerald-100" : "bg-rose-100"}`}>
              {isPaid
                ...
                : <FiDollarSign size={13} className="text-rose-600" />
              }
            </div>
            <div>
          ...
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gradient-to-r from-emerald-50 via-teal-50 to-cyan-50 border border-emerald-200">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[9px] text-emerald-500 font-medium mt-0.5">
                {savingsPct}% off the original price
              </p>
            </div>
...
/* ================================================================
   ITEM CARD — Desktop: Premium redesign with two-row layout
   ================================================================ */
const ItemCard = ({
  ...
  return (
    <div className="hidden xl:block bg-white border border-slate-200 rounded-2xl overflow-visible transition-all duration-200 hover:border-indigo-200 hover:shadow-lg group">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                {item.product_brand && (
                  <span className="text-[9px] font-semibold text-slate-400">
                    {capitalizeFirstLetter(item.product_brand)}
                  </span>
                )}
                {item.product_model && (
                  <>
                    ...
                    <span className="text-[9px] font-semibold text-slate-400">
                      {capitalizeFirstLetter(item.product_model)}
                    </span>
                  </>
                )}
                {item.product_type && (
            ...
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-50 border border-indigo-100">
              {item.is_product_scheme ? (
                ...
                <>
                  ...
                  {discountLabel && (
                    <span className="text-[9px] font-black text-rose-500 bg-rose-50 px-1 rounded">
                      {discountLabel}
                    </span>
                  )}
                </>
      ...
      <div className="px-5 pt-4 pb-5 space-y-4">
        {/* ── ROW 1: Brand + Model with visual separator ── */}
        <div className="grid grid-cols-2 gap-4 pb-4 border-b border-dashed border-slate-100">
          <div>
            ...
            <CascadeSelect
              value={currentBrand} options={brandOptions}
          ...
          <div>
            ...
            <CascadeSelect
              value={currentModel} options={modelOptions}
  ...
  <div className="xl:hidden bg-white border border-slate-200 rounded-2xl overflow-hidden">
    <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100">
      ...
      <div className="flex items-center gap-2">
        <button type="button" onClick={() => onItemChange(index, "is_product_scheme", !item.is_product_scheme)}
          className={`flex items-center gap-1 px-2 py-1 rounded border text-[9px] font-black uppercase tracking-wide transition-all ${item.is_product_scheme ? "bg-gradient-to-r from-emerald-500 to-teal-500 text-white border-transparent" : "bg-white text-slate-400 border-slate-200 hover:border-indigo-300"
            }`}>
          <FiZap size={8} />Scheme
        </button>
        {totalItems > 1 && (
          <button type="button" onClick={() => onRemove(index)} className="p-1.5 rounded-lg text-rose-400 hover:text-rose-600 hover:bg-rose-50 transition-all">
            <FiTrash2 size={12} />
          </button>
        )}
      </div>
    ...
    <div className="p-4 space-y-3">
      ...
      <Field label="Product" required>
        <ProductDropdown value={item.product_id} options={itemProductOptions} onChange={(e) => onItemChange(index, "product_id", e.target.value)}
          placeholder={currentBrand ? "Search product…" : "Select brand first"} isLoading={loadingProducts} productsMap={productsMap} />
        {stockInfo && item.product_id && <StockIndicator stockInfo={stockInfo} qty={item.qty_ordered} />}
      </Field>
      <div className="grid grid-cols-2 gap-3">
        <Field label="Quantity" required>
          <StyledInput type="number" min="1" value={item.qty_ordered} onChange={(e) => onItemChange(index, "qty_ordered", e.target.value)} className="text-center font-bold" />
        </Field>
...
const CreateOrder = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const requests = [fetchDealers(), ...(canSelectSalesmanPermission ? [fetchUserByRole(ROLES.SALESMAN)] : [])];
  ...
  useEffect(() => {
    const load = async () => {
      if (!formData.dealer_id) {
        setProducts([]); setAllBrands([]); setItemBrands({}); setItemModels({});
        return;
      }
      setLoadingProducts(true);
      try {
        const brandRes = await getBrandsByDealer(formData.dealer_id, "active");
        if (!brandRes?.success || !brandRes?.data?.length) { setProducts([]); setAllBrands([]); return; }
  ...
  const getItemProductOptions = useCallback((index) => {
    ...
    return products
      ...
      .map((p) => ({ value: p.product_id, label: p.product_name, product_name: p.product_name, product_model: p.model, product_type: p.product_type, product_brand: p.brand }));
  }, [products, itemBrands, itemModels]);

  const getBrandOptions = useCallback(() => allBrands.map((b) => ({ value: b.brand_name, label: b.brand_name })), [allBrands]);
  ...
  const handleItemBrandChange = (index, brandName) => {
    ...
    updatedItems[index] = { ...updatedItems[index], product_id: "", product_brand: "", product_name: "", product_model: "", product_type: "", product_price: 0, available_stock: 0, packed_stock: 0, unpacked_stock: 0, discount_price: 0, dealer_discount_id: null, delivery_date: "" };
  ...
  const handleItemChange = async (index, field, value) => {
    ...
    if (field === "product_id") {
      ...
      if (selectedProduct) {
        updatedItems[index] = {
          ...updatedItems[index],
          product_id: selectedProduct.product_id, product_brand: selectedProduct.brand,
          product_name: selectedProduct.product_name, product_model: selectedProduct.model,
          product_type: selectedProduct.product_type, product_price: selectedProduct.price,
          ...
          discount_price: 0, dealer_discount_id: null, delivery_date: getMinDeliveryDate(),
        };
        if (!itemBrands[index] && selectedProduct.brand) setItemBrands((prev) => ({ ...prev, [index]: selectedProduct.brand }));
        if (!itemModels[index] && selectedProduct.model) setItemModels((prev) => ({ ...prev, [index]: selectedProduct.model }));
        const discountRes = await fetchDealerDiscounts({ dealer_id: formData.dealer_id, product_id: value });
        setDiscountOptions((prev) => ({ ...prev, [index]: discountRes?.success && discountRes?.data?.length ? discountRes.data : [] }));
      }
    }
    setFormData((prev) => ({ ...prev, order_details: updatedItems }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const validItems = formData.order_details
        ...
        .map((item) => {
          const p = {
            product_id: item.product_id, product_brand: item.product_brand, product_name: item.product_name,
            product_model: item.product_model, product_type: item.product_type, product_price: item.product_price,
            qty_ordered: Number(item.qty_ordered), delivery_date: item.delivery_date, is_product_scheme: item.is_product_scheme,
          };
      ...
      if (!validItems.length) { setError("Add at least one valid product"); setLoading(false); return; }

      const payload = {
        dealer_id: formData.dealer_id, priority: formData.priority, order_note: formData.order_note,
        salesman_id: canSelectSalesmanPermission ? formData.salesman_id : user.employee_id,
        amount_paid: Number(formData.amount_paid) || 0, payment_method: formData.payment_method,
  ...
  const financialSummary = useMemo(() => {
    ...
    formData.order_details.forEach((item, index) => {
      ...
      if (dd) totalDiscount += dd.is_percentage ? (itemTotal * dd.discount_value) / 100 : Number(dd.discount_value || 0) * qty;
      else if (item.discount_price > 0) totalDiscount += Number(item.discount_price || 0) * qty;
    });
            ...
            <div className="space-y-5">
              <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
                <Field label="Dealer" required>
                  <CustomSelect name="dealer_id" value={formData.dealer_id} onChange={handleChange}
                    options={dealers.map((d) => ({ value: d.employee_id, label: `${capitalizeFirstLetter(d.employee_name)} — ${capitalizeFirstLetter(d.shop_name)}` }))}
                ...
                {canSelectSalesmanPermission && (
                  <Field label="Salesman" required>
                    <CustomSelect name="salesman_id" value={formData.salesman_id} onChange={handleChange}
                ...
                <Field label="Amount Paid" hint="Optional">
                  <div className="relative">
                    ...
                    <StyledInput type="number" name="amount_paid" value={formData.amount_paid} onChange={handleChange} placeholder="0" min={0} className="pl-7" />
                  </div>
                </Field>
            ...
            <div className="space-y-3">
              ...
              {!formData.dealer_id && (
                <div className="flex items-center gap-3 px-4 py-3 bg-amber-50 border border-amber-200 border-dashed rounded-xl">
                  ...
                  <p className="text-xs font-semibold text-amber-700">Select a dealer first to load available brands and products</p>
                </div>
              )}
              ...
              {formData.order_details.map((item, index) => {
                ...
                let finalAmount = itemTotal, discountLabel = null;
                if (!item.is_product_scheme) {
                  if (dealerDiscount) {
                    const da = dealerDiscount.is_percentage ? (itemTotal * dealerDiscount.discount_value) / 100 : Number(dealerDiscount.discount_value || 0) * qty;
                    finalAmount -= da;
                    discountLabel = dealerDiscount.is_percentage ? `${dealerDiscount.discount_value}% off` : `₹ ${da.toLocaleString("en-IN")} off`;
                ...
                const commonProps = {
                  ...
                  discountOptions, productsMap, loadingProducts, dealerSelected: !!formData.dealer_id,
                  getMinDeliveryDate, itemTotal, finalAmount, discountLabel,
                  onBrandChange: handleItemBrandChange, onModelChange: handleItemModelChange,
                  onItemChange: handleItemChange, onDealerDiscount: handleDealerDiscount,
                  onManualDiscount: handleManualDiscount, onClearDiscount: handleClearDiscount,
                  onRemove: removeItem,
                };
