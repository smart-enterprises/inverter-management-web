...
/** Labelled info row in the dealer profile card */
...
/** Modal backdrop + centred container */
...
/* ─────────────────────────────────────────────────────────────────────────────
   ...
   Uses ref-based outside-click so selecting a product does NOT close the panel.
───────────────────────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────────────────────
   BRAND MULTI-SELECT  (reused for Add Brand & Models modal)
───────────────────────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────────────────────
   ADD BRAND & MODELS MODAL
   Allows selecting new brands to assign, then triggers updateDealer API.
───────────────────────────────────────────────────────────────────────────── */
const AddBrandModelsModal = ({ dealerId, existingBrands = [], onClose, onSuccess }) => {
  ...
  // Exclude brands already assigned
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Brands Added",
        text: res.message || "Brands assigned successfully.",
      });
    } catch (err) {
    ...
    <ModalShell maxWidth="max-w-lg">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
        ...
        <FormField label="Select Brands to Add" required>
          <BrandMultiSelect
            ...
            searchable
          />
          {availableOptions.length === 0 && !brandsLoading && (
      ...
      <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/30 flex gap-3 flex-shrink-0">
        ...
        <button
          ...
          disabled={submitting || !selectedBrands.length}
    ...
    <ModalShell maxWidth="max-w-5xl">
      ...
      <div className="flex-1 overflow-y-auto px-7 py-6 space-y-5">
        ...
        {rows.map((row, idx) => (
          <div
            key={row.id}
            className="bg-slate-50 rounded-xl border border-slate-200 p-6 space-y-5"
          >
...
const DealerDetails = () => {
  ...
  /* ── Modal visibility ── */
  const [activeModal, setActiveModal] = useState(null); // 'addBrand' | 'addDiscount' | 'editDiscount'
  ...
  const loadDealerData = useCallback(async () => {
    ...
    try {
      ...
      const ordersRes = await fetchOrders({
        ...
        dealer: id,
        status:
          orderFilters.selectedStatus !== "ALL"
            ? orderFilters.selectedStatus.toUpperCase()
            : undefined,
        priority:
          orderFilters.selectedPriority !== "ALL"
            ? orderFilters.selectedPriority.toUpperCase()
            : undefined,
        search: orderFilters.searchQuery || undefined,
      });
  ...
  }, [id, pagination.page, pagination.limit, orderFilters]);
  ...
  const handleSearchChange = useCallback((value) => {
    setOrderFilters((prev) => ({ ...prev, searchQuery: value }));
    setPagination((prev) => ({ ...prev, page: 1 }));
  }, []);

  const handleStatusChange = useCallback((value) => {
    setOrderFilters((prev) => ({ ...prev, selectedStatus: value }));
    setPagination((prev) => ({ ...prev, page: 1 }));
  }, []);

  const handlePriorityChange = useCallback((value) => {
    setOrderFilters((prev) => ({ ...prev, selectedPriority: value }));
    setPagination((prev) => ({ ...prev, page: 1 }));
  }, []);
    ...
    <div className="min-h-screen bg-slate-50/60 p-4 sm:p-6 lg:p-8">
      <div className="max-w-screen-xl mx-auto space-y-5">
        ...
        <OrderHistorySection
          ...
          selectedStatus={orderFilters.selectedStatus}
          selectedPriority={orderFilters.selectedPriority}
      ...
      {showDiscountActions &&
        ...
        selectedDiscount && (
          <EditDiscountModal
            ...
            onChange={(patch) =>
              setSelectedDiscount((prev) => ({ ...prev, ...patch }))
            }
          />
        )}
    </div>
