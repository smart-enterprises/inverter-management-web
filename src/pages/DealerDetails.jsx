...
/* ================================================================
   HELPERS
   ================================================================ */
const createEmptyRow = () => ({ id: crypto.randomUUID(), brand_name: "", model_name: "", product_ids: [], discount_value: "", is_percentage: true, description: "" });
...
const getTotalItems = (details = []) => details.reduce((acc, item) => acc + (item.qty_ordered || 0), 0);
...
const StatusBadge = ({ status }) => {
  ...
  return <span className={`inline-flex items-center px-2.5 py-1 text-[10px] font-black rounded-full border uppercase tracking-wide ${style}`}>{status}</span>;
};
const PriorityBadge = ({ priority }) => {
  const style = getPriorityStyle(priority);
  return <span className={`inline-flex items-center px-2.5 py-1 text-[10px] font-black rounded-full border uppercase tracking-wide ${style}`}>{priority || "N/A"}</span>;
};

/* ================================================================
   PRODUCT MULTI-SELECT  (logic unchanged, styled)
   ================================================================ */
function ProductMultiSelect({ products, selected, onChange }) {
  ...
  const toggleProduct = (id) => onChange(selected.includes(id) ? selected.filter((x) => x !== id) : [...selected, id]);
        ...
        {selected.map((id) => {
          ...
          return (
            <span key={id} className="flex items-center gap-1 bg-indigo-50 text-indigo-700 border border-indigo-200 px-2 py-0.5 rounded-full text-[10px] font-black">
              ...
              <button onClick={(e) => { e.stopPropagation(); toggleProduct(id); }} className="text-indigo-400 hover:text-indigo-700"><FiX size={9} /></button>
            </span>
          );
        })}
      </div>
        ...
        <div className="absolute z-40 mt-1.5 w-full bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="max-h-52 overflow-y-auto">
            {filtered.map((product) => {
              ...
              return (
                <div key={product.product_id} onClick={() => toggleProduct(product.product_id)}
...
const DealerDetails = () => {
  ...
  const buildBrandModelMap = (brands = []) => brands.reduce((map, brand) => {
  ...
  const loadDealerData = useCallback(async () => {
    ...
    try {
      ...
      const ordersRes = await fetchOrders({ page: pagination.page, limit: pagination.limit, dealer: id, status: selectedStatus && selectedStatus !== "ALL" ? selectedStatus.toUpperCase() : undefined, priority: selectedPriority && selectedPriority !== "ALL" ? selectedPriority.toUpperCase() : undefined, search: searchQuery || undefined });
      if (ordersRes?.success) { setDealerOrders(ordersRes.data || []); setPagination((prev) => ({ ...prev, total: ordersRes.pagination?.total || 0 })); }
      else setDealerOrders([]);
  ...
  const loadDiscounts = useCallback(async (page = 1) => {
    ...
    } catch (err) { setDiscounts([]); setDiscountState((s) => ({ ...s, total: 0, error: err.message })); }
    finally { setDiscountState((s) => ({ ...s, loading: false })); }
  ...
  const loadOrderSummary = useCallback(async () => {
    ...
    try {
      const statuses = [{ key: "total", value: null }, { key: "pending", value: "PENDING" }, { key: "inProduction", value: "PRODUCTION" }, { key: "inPacking", value: "PACKED" }, { key: "delivered", value: "DELIVERED" }, { key: "cancelled", value: "CANCELLED" }];
      ...
      for (const statusObj of statuses) {
        ...
        } catch (err) { console.error(`Failed for status ${statusObj.value}`, err); }
      }
      setOrderStats(stats);
    } catch (err) { console.error("Order summary load failed:", err); }
    finally { loadOrderSummary.loading = false; }
  }, [id]);
  ...
  const handleSubmitDiscounts = async () => {
    try {
      ...
      const payload = bulkRows.filter((r) => r.brand_name && r.model_name && r.discount_value).map((r) => ({ dealer_id: id, brand_name: r.brand_name, model_name: r.model_name, product_ids: r.product_ids, discount_value: Number(r.discount_value), is_percentage: Boolean(r.is_percentage), description: r.description?.trim() || "" }));
      ...
      setBulkModalOpen(false); setBulkRows([createEmptyRow()]);
  ...
  const handleUpdateDiscount = async () => {
    try {
      ...
      setEditDiscountModalOpen(false); await loadDiscounts(discountState.page);
  ...
  const openEditDiscountModal = async (discount) => {
    ...
    setSelectedDiscount(cleanDiscount); setOriginalDiscount(cleanDiscount);
    if (!productsByBrand[discount.brand_name]) {
      try { const res = await fetchProductsByBrands([discount.brand_name]); if (res?.success) setProductsByBrand((prev) => ({ ...prev, [discount.brand_name]: res.data })); }
      ...
      <div className="max-w-screen-xl mx-auto space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3.5">
            <button onClick={() => navigate("/dealers")} className="p-2 rounded-xl border border-slate-200 bg-white hover:border-slate-300 hover:shadow-sm transition-all group">
          ...
          <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[10px] font-black uppercase tracking-wide ${dealer.status?.toLowerCase() === "active" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-rose-50 text-rose-700 border-rose-200"}`}>
                    ...
                    <div key={index} className="relative pl-4">
                      ...
                      {models.length > 0 ? (
                        <ul className="space-y-1">
                          {models.map((model, i) => <li key={i} className="text-xs text-slate-500 font-medium italic">{model}</li>)}
                        </ul>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/50">
            ...
            <button onClick={() => setBulkModalOpen(true)} className="inline-flex items-center gap-2 px-4 py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-xl hover:bg-indigo-700 active:scale-95 transition-all shadow-sm shadow-indigo-200">
          ...
          <div className="p-6">
            {discountState.loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 gap-3">
                ...
                <p className="text-xs text-slate-400">Start by adding pricing rules for this dealer.</p>
              </div>
            ) : (
              <>
                  ...
                  <table className="min-w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-100 bg-slate-50/60">
                        {["Brand", "Model", "Products", "Discount", "Status", "Created", ""].map((h, i) => (
                          <th key={i} className={`px-5 py-3.5 text-[10px] font-black uppercase tracking-[0.1em] text-slate-400 ${i === 6 ? "text-right" : "text-left"} whitespace-nowrap`}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                        ...
                        <tr key={d.dealer_discount_id} className="hover:bg-slate-50/60 transition-colors">
                            ...
                            {d.products && d.products.length > 0 ? (
                              <div className="flex flex-wrap gap-1.5 max-w-[280px] max-h-[60px] overflow-y-auto">
                                {d.products.map((p) => {
                                  ...
                                  const discountAmount = d.is_percentage ? (originalPrice * d.discount_value) / 100 : d.discount_value;
                          ...
                          <td className="px-5 py-4">
                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-black uppercase tracking-wide ${d.status === "active" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-slate-100 text-slate-600 border-slate-200"}`}>
                              <span className={`w-1.5 h-1.5 rounded-full ${d.status === "active" ? "bg-emerald-500" : "bg-slate-400"}`} />{d.status}
                            </span>
                          </td>
                ...
                {discountState.total > 0 && (
                  <MiniPagination page={discountState.page} total={discountState.total} limit={discountState.limit} onPageChange={(p) => loadDiscounts(p)} />
                )}
              </>
            )}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-slate-100 flex flex-col lg:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Search orders…" value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setPagination((prev) => ({ ...prev, page: 1 })); }}
            ...
            <div className="flex gap-2.5">
              <div className="w-44"><CustomSelect name="status" value={selectedStatus} onChange={(e) => { setSelectedStatus(e.target.value); setPagination((prev) => ({ ...prev, page: 1 })); }} options={ORDER_STATUS_LIST} /></div>
              <div className="w-44"><CustomSelect name="priority" value={selectedPriority} onChange={(e) => { setSelectedPriority(e.target.value); setPagination((prev) => ({ ...prev, page: 1 })); }} options={PRIORITY_OPTIONS} /></div>
            </div>
          </div>
          <div className="p-6">
              ...
              <>
                <div className="overflow-x-auto rounded-xl border border-slate-200">
                  <table className="min-w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-100 bg-slate-50/60">
                        {["Order ID", "Date", "Items", "Priority", "Status", ""].map((h, i) => (
                          <th key={i} className={`px-5 py-3.5 text-[10px] font-black uppercase tracking-[0.1em] text-slate-400 whitespace-nowrap ${i === 5 ? "text-right" : "text-left"}`}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                      {dealerOrders.map(({ order }) => (
                        <tr key={order.order_number} className="hover:bg-slate-50/60 transition-colors">
                          ...
                          <td className="px-5 py-4 text-right">
                            <button onClick={() => navigate(`/orders/${order.order_number}`)} className="text-indigo-600 font-bold hover:text-indigo-800 text-sm transition-colors">
                              View →
                            </button>
                          </td>
                        </tr>
                ...
                {pagination.total > 0 && (
                  <MiniPagination page={pagination.page} total={pagination.total} limit={pagination.limit} onPageChange={(p) => setPagination((prev) => ({ ...prev, page: p }))} />
                )}
              </>
            )}
          </div>
      ...
      {bulkModalOpen && (
        <>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4 sm:p-6">
            <div className="w-full max-w-5xl bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col" style={{ maxHeight: "92vh" }}>
              <div className="px-7 py-6 border-b border-slate-100 bg-slate-50/50 flex-shrink-0">
                ...
                <p className="text-xs text-slate-400 font-medium mt-0.5">Configure pricing rules across brands, models and products.</p>
              </div>
              <div className="flex-1 overflow-y-auto px-7 py-6 space-y-5">
                {bulkError && <div className="flex items-center gap-2.5 px-4 py-3 bg-rose-50 border border-rose-200 text-rose-700 rounded-xl text-sm font-semibold"><FiAlertCircle size={14} />{bulkError}</div>}
                {bulkRows.map((row, idx) => (
                  <div key={row.id} className="bg-slate-50 rounded-xl border border-slate-200 p-6 space-y-5">
                    <div className="flex items-center justify-between">
                      ...
                      {bulkRows.length > 1 && (
                        <button onClick={() => setBulkRows((prev) => prev.filter((_, i) => i !== idx))} className="p-1.5 rounded-lg text-rose-400 hover:text-rose-600 hover:bg-rose-50 transition-all"><FiTrash2 size={13} /></button>
                      )}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      <FormField label="Brand">
                        <CustomSelect value={row.brand_name}
                          onChange={async (e) => {
                            const selectedBrand = e.target.value;
                            setBulkRows((prev) => prev.map((r, i) => i === idx ? { ...r, brand_name: selectedBrand, model_name: "", product_ids: [] } : r));
                            if (!productsByBrand[selectedBrand]) {
                              try { const res = await fetchProductsByBrands([selectedBrand]); if (res?.success && res?.data) setProductsByBrand((prev) => ({ ...prev, [selectedBrand]: res.data })); }
                              catch (err) { console.error("Product fetch failed:", err); }
                            }
                          }}
                          options={allBrands.map((b) => b.brand_name)} placeholder="Select Brand"
                        />
                      </FormField>
                      <FormField label="Model">
                        <CustomSelect value={row.model_name} onChange={(e) => setBulkRows((prev) => prev.map((r, i) => i === idx ? { ...r, model_name: e.target.value } : r))} options={row.brand_name ? brandToModels[row.brand_name] || [] : []} placeholder="Select Model" disabled={!row.brand_name} />
                      </FormField>
                      <FormField label="Discount">
                        <div className="flex items-center gap-2">
                          <input type="number" min="0" max={row.is_percentage ? 100 : undefined} value={row.discount_value}
                            onChange={(e) => { let value = e.target.value; if (row.is_percentage && Number(value) > 100) value = 100; setBulkRows((prev) => prev.map((r, i) => i === idx ? { ...r, discount_value: value } : r)); }}
                          ...
                          <div className="flex bg-slate-100 rounded-full p-1 flex-shrink-0">
                            <button type="button" onClick={() => setBulkRows((prev) => prev.map((r, i) => i === idx ? { ...r, is_percentage: false } : r))} className={`px-3 py-1.5 text-xs font-bold rounded-full transition ${!row.is_percentage ? "bg-white shadow text-indigo-600" : "text-slate-500"}`}>₹</button>
                            <button type="button" onClick={() => setBulkRows((prev) => prev.map((r, i) => i === idx ? { ...r, is_percentage: true } : r))} className={`px-3 py-1.5 text-xs font-bold rounded-full transition ${row.is_percentage ? "bg-white shadow text-indigo-600" : "text-slate-500"}`}>%</button>
                          </div>
                        </div>
                      </FormField>
                      <FormField label="Products">
                        <ProductMultiSelect
                          ...
                          selected={row.product_ids || []} onChange={(ids) => setBulkRows((prev) => prev.map((r, i) => i === idx ? { ...r, product_ids: ids } : r))}
                        />
                      </FormField>
                    </div>
                    <FormField label="Description (Optional)">
                      <textarea rows={2} maxLength={200} value={row.description} onChange={(e) => setBulkRows((prev) => prev.map((r, i) => i === idx ? { ...r, description: e.target.value } : r))} placeholder="Enter description…"
              ...
              <div className="px-7 py-5 border-t border-slate-100 bg-slate-50/30 flex justify-between items-center flex-shrink-0">
                <button onClick={() => setBulkRows((prev) => [...prev, createEmptyRow()])} className="inline-flex items-center gap-1.5 px-4 py-2.5 border border-indigo-300 text-indigo-600 rounded-xl text-sm font-bold hover:bg-indigo-50 transition-all">
                  <FiPlus size={13} />Add Rule
                </button>
                <div className="flex gap-3">
                  <button onClick={() => setBulkModalOpen(false)} className="px-5 py-2.5 border border-slate-200 rounded-xl text-slate-600 text-sm font-semibold hover:bg-slate-50 transition-all">Cancel</button>
                  <button disabled={bulkSubmitting} onClick={handleSubmitDiscounts} className="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 active:scale-95 transition-all disabled:opacity-60 shadow-sm shadow-indigo-200">
                    {bulkSubmitting ? <><div className="w-3.5 h-3.5 border-2 border-white/40 border-t-white rounded-full animate-spin" />Saving…</> : "Save Discounts"}
                  </button>
                </div>
              </div>
            </div>
      ...
      {editDiscountModalOpen && selectedDiscount && (
        <>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
            <div className="w-full max-w-xl bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col" style={{ maxHeight: "90vh" }}>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <FormField label="Brand"><input value={selectedDiscount.brand_name} disabled className={inputCls} /></FormField>
                  <FormField label="Model"><input value={selectedDiscount.model_name} disabled className={inputCls} /></FormField>
                </div>
                <FormField label="Discount">
                  <div className="flex items-center gap-2">
                    <input type="number" value={selectedDiscount.discount_value} onChange={(e) => setSelectedDiscount((prev) => ({ ...prev, discount_value: e.target.value }))} className="flex-1 border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm font-medium text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 transition-all" />
                    <div className="flex bg-slate-100 rounded-full p-1">
                      <button onClick={() => setSelectedDiscount((prev) => ({ ...prev, is_percentage: false }))} className={`px-3 py-1.5 text-xs font-bold rounded-full transition ${!selectedDiscount.is_percentage ? "bg-white shadow text-indigo-600" : "text-slate-500"}`}>₹</button>
                      <button onClick={() => setSelectedDiscount((prev) => ({ ...prev, is_percentage: true }))} className={`px-3 py-1.5 text-xs font-bold rounded-full transition ${selectedDiscount.is_percentage ? "bg-white shadow text-indigo-600" : "text-slate-500"}`}>%</button>
                    </div>
                  </div>
                </FormField>
                <FormField label="Products">
                  <ProductMultiSelect
                    products={(productsByBrand[selectedDiscount.brand_name] || []).filter((p) => selectedDiscount.model_name ? p.model === selectedDiscount.model_name : true)}
                    selected={selectedDiscount.product_ids || []} onChange={(ids) => setSelectedDiscount((prev) => ({ ...prev, product_ids: ids }))}
                  />
                </FormField>
                <FormField label="Description">
                  <textarea rows={3} value={selectedDiscount.description && selectedDiscount.description.toLowerCase() !== "null" ? selectedDiscount.description : ""} onChange={(e) => setSelectedDiscount((prev) => ({ ...prev, description: e.target.value }))} placeholder="Enter description…"
                    className="w-full border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm font-medium text-slate-800 placeholder-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 transition-all resize-none"
                  />
                </FormField>
              </div>
              <div className="px-6 py-4 border-t border-slate-100 flex gap-3 flex-shrink-0">
                <button onClick={() => setEditDiscountModalOpen(false)} className="flex-1 py-2.5 border border-slate-200 rounded-xl text-slate-600 text-sm font-semibold hover:bg-slate-50 transition-all">Cancel</button>
                <button onClick={handleUpdateDiscount} className="flex-1 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 active:scale-95 transition-all shadow-sm shadow-indigo-200">Update Discount</button>
              </div>
            </div>
          </div>
        </>
