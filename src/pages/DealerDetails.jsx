...
import {
  ...
  FiChevronDown, FiRefreshCw, FiArrowRight, FiFilter,
} from "react-icons/fi";
...
const createEmptyDiscountRow = () => ({
  id: crypto.randomUUID(), brand_name: "", model_name: "",
  product_ids: [], discount_value: "", is_percentage: true, description: "",
});
  ...
  <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
    <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/50">
      <div>
        ...
        {subtitle && <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-[0.1em] mt-0.5">{subtitle}</p>}
      </div>
      {action}
    </div>
...
const TextInput = ({ className = "", ...props }) => (
  <input
    {...props}
    className={`w-full border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm font-medium text-slate-800 placeholder-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 transition-all disabled:bg-slate-50 disabled:text-slate-400 ${className}`}
  />
);
...
const AlertBanner = ({ message }) =>
  message ? (
    <div className="flex items-center gap-2.5 px-4 py-3 bg-rose-50 border border-rose-200 text-rose-700 rounded-xl text-sm font-semibold">
      <FiAlertCircle size={14} className="flex-shrink-0" />{message}
    </div>
  ) : null;
...
const ModalShell = ({ maxWidth = "max-w-xl", children }) => (
  <>
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 p-4 sm:p-6">
      <div className={`w-full ${maxWidth} bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col`} style={{ maxHeight: "92vh" }}>
        {children}
      </div>
    </div>
  </>
...
const ModalHeader = ({ title, subtitle, onClose }) => (
  <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/50 flex-shrink-0 flex items-start justify-between">
    ...
    <button onClick={onClose} className="p-2 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-all flex-shrink-0 ml-4" aria-label="Close">
      <FiX size={16} />
    </button>
  </div>
);
...
const DateRangeFilter = ({ startDate, endDate, onStartChange, onEndChange }) => (
  <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5">
    ...
    <div className="flex flex-col">
      ...
      <input
        type="date" value={startDate} onChange={onStartChange}
    ...
    <div className="flex flex-col">
      ...
      <input
        type="date" value={endDate} onChange={onEndChange}
        className="text-[11px] font-medium text-slate-700 bg-transparent outline-none cursor-pointer min-w-[110px]"
      />
    </div>
    {(startDate || endDate) && (
...
const ProductMultiSelect = ({ products = [], selected = [], onChange }) => {
  ...
  return (
    <div className="relative" ref={containerRef}>
      <div onClick={() => setOpen((prev) => !prev)} className="min-h-[44px] flex flex-wrap gap-1.5 px-3.5 py-2.5 border border-slate-200 rounded-lg cursor-pointer hover:border-indigo-300 transition-colors">
        {selected.length === 0 && <span className="text-slate-400 text-sm font-medium">Select products…</span>}
        {selected.map((id) => {
          const product = products.find((p) => p.product_id === id);
          if (!product) return null;
          return (
            <span key={id} className="flex items-center gap-1 bg-indigo-50 text-indigo-700 border border-indigo-200 px-2 py-0.5 rounded-full text-[10px] font-black">
              {product.product_name}
              <button onMouseDown={(e) => { e.stopPropagation(); toggle(id); }} className="text-indigo-400 hover:text-indigo-700"><FiX size={9} /></button>
            </span>
          );
        })}
      </div>
      {open && (
        <div className="absolute z-40 mt-1.5 w-full bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden">
          <div className="p-2 border-b border-slate-100">
            <input placeholder="Search products…" value={search} onChange={(e) => setSearch(e.target.value)} onMouseDown={(e) => e.stopPropagation()}
          ...
          <div className="max-h-52 overflow-y-auto">
            {filtered.length === 0 ? <div className="px-4 py-5 text-sm text-slate-400 text-center font-semibold">No products found</div> : (
              filtered.map((product) => {
                const active = selected.includes(product.product_id);
                return (
                  <div key={product.product_id} onMouseDown={(e) => { e.stopPropagation(); toggle(product.product_id); }}
...
/* ─────────────────────────────────────────────────────────────────────
   BRAND MULTI-SELECT
───────────────────────────────────────────────────────────────────────*/
const BrandMultiSelect = ({ options = [], selectedValues = [], onChange, placeholder = "Select brands…", disabled = false, loading = false, searchable = false }) => {
  const [open, setOpen] = useState(false);
  ...
  const toggle = (value) => onChange(selectedValues.includes(value) ? selectedValues.filter((v) => v !== value) : [...selectedValues, value]);
  const removeTag = (value) => onChange(selectedValues.filter((v) => v !== value));

  return (
    <div className="relative" ref={containerRef}>
      <div onClick={() => !disabled && !loading && setOpen((p) => !p)}
        className={`w-full px-3.5 py-2.5 rounded-lg border min-h-[42px] flex flex-wrap items-center gap-1.5 cursor-pointer transition-all ${disabled ? "opacity-50 cursor-not-allowed bg-slate-50 border-slate-200" : "bg-white border-slate-200 hover:border-indigo-300"} ${open ? "border-indigo-400 ring-2 ring-indigo-100" : ""}`}>
        {loading ? (
          <div className="flex items-center gap-2"><div className="w-3.5 h-3.5 border-2 border-indigo-200 border-t-indigo-600 rounded-full animate-spin" /><span className="text-sm text-slate-400">{placeholder}</span></div>
        ) : selectedValues.length > 0 ? (
          selectedValues.map((value) => {
            const option = options.find((o) => o.value === value);
            return (
              <span key={value} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black bg-indigo-50 text-indigo-700 border border-indigo-200 max-w-[120px]">
                <span className="truncate">{option?.label || value}</span>
                <button type="button" onMouseDown={(e) => { e.stopPropagation(); removeTag(value); }} className="text-indigo-400 hover:text-indigo-700 flex-shrink-0"><FiX size={9} /></button>
              </span>
            );
          })
      ...
      {open && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg max-h-60 overflow-hidden">
          {searchable && (
            <div className="p-2 border-b border-slate-100">
              <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onMouseDown={(e) => e.stopPropagation()} placeholder="Search brands…"
                className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400" />
            </div>
          )}
          <div className="max-h-48 overflow-y-auto">
            {filtered.length > 0 ? filtered.map((option) => {
              ...
              return (
                <div key={option.value} onMouseDown={(e) => { e.stopPropagation(); toggle(option.value); }}
                  ...
                  <span className={`font-medium ${active ? "text-indigo-700" : "text-slate-700"}`}>{option.label}</span>
                  {active && <FiCheck className="text-indigo-600 flex-shrink-0" size={14} />}
                </div>
              );
            }) : <div className="px-4 py-5 text-sm text-slate-400 text-center font-semibold">{searchTerm ? "No brands found" : "No brands available"}</div>}
          </div>
        </div>
      )}
    </div>
...
const AddBrandModelsModal = ({ dealerId, existingBrands = [], onClose, onSuccess }) => {
  ...
  const availableOptions = useMemo(
    () => allBrands.filter((b) => !existingBrands.includes(b.brand_name)).map((b) => ({ value: b.brand_name, label: b.brand_name })),
    [allBrands, existingBrands]
  );

  const handleSubmit = async () => {
    setError("");
    if (!selectedBrands.length) { setError("Please select at least one brand."); return; }
    try {
      setSubmitting(true);
  ...
  return (
    <ModalShell maxWidth="max-w-lg">
      <ModalHeader title="Add Brands" subtitle="Assign new brands to this dealer account" onClose={onClose} />
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
        <AlertBanner message={error} />
        {existingBrands.length > 0 && (
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-400 mb-2">Currently Assigned</p>
            <div className="flex flex-wrap gap-1.5">
              {existingBrands.map((b) => (
                <span key={b} className="inline-flex items-center px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 border border-slate-200 text-[10px] font-black uppercase tracking-wide">{b}</span>
              ))}
            </div>
          </div>
        )}
        <FormField label="Select Brands to Add" required>
          <BrandMultiSelect
            options={availableOptions} selectedValues={selectedBrands} onChange={setSelectedBrands}
            placeholder={brandsLoading ? "Loading brands…" : availableOptions.length === 0 ? "All brands already assigned" : "Select brands…"}
            disabled={brandsLoading || availableOptions.length === 0} loading={brandsLoading} searchable
          />
          {availableOptions.length === 0 && !brandsLoading && <p className="text-xs text-slate-400 italic mt-1">All available brands are already assigned to this dealer.</p>}
        </FormField>
      </div>
      <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/30 flex gap-3 flex-shrink-0">
        <button onClick={onClose} className="flex-1 py-2.5 border border-slate-200 rounded-xl text-slate-600 text-sm font-semibold hover:bg-slate-50 transition-all">Cancel</button>
        <button onClick={handleSubmit} disabled={submitting || !selectedBrands.length}
          ...
          {submitting ? <><div className="w-3.5 h-3.5 border-2 border-white/40 border-t-white rounded-full animate-spin" />Saving…</> : "Add Brands"}
        </button>
      </div>
    </ModalShell>
  );
...
const DiscountToggle = ({ isPercentage, onChange }) => (
  <div className="flex bg-slate-100 rounded-full p-1 flex-shrink-0">
    <button type="button" onClick={() => onChange(false)}
      className={`px-3 py-1.5 text-xs font-bold rounded-full transition ${!isPercentage ? "bg-white shadow text-indigo-600" : "text-slate-500"}`}>₹</button>
...
const AddDiscountsModal = ({ dealerId, allBrands, brandToModels, onClose, onSuccess }) => {
  ...
  const fetchBrandProducts = async (brandName) => {
    ...
    try {
      ...
      if (res?.success && res.data) setProductsByBrand((prev) => ({ ...prev, [brandName]: res.data }));
  ...
  const handleSubmit = async () => {
    setError("");
    const payload = rows.filter((r) => r.brand_name && r.model_name && r.discount_value).map((r) => ({
      dealer_id: dealerId, brand_name: r.brand_name, model_name: r.model_name,
      product_ids: r.product_ids, discount_value: Number(r.discount_value),
      is_percentage: Boolean(r.is_percentage), description: r.description?.trim() || "",
    }));
    if (!payload.length) { setError("Please configure at least one valid discount rule."); return; }
    try {
      setSubmitting(true);
  ...
  return (
    <ModalShell maxWidth="max-w-5xl">
      <ModalHeader title="Add Dealer Discounts" subtitle="Configure pricing rules across brands, models and products" onClose={onClose} />
      <div className="flex-1 overflow-y-auto px-7 py-6 space-y-5">
        <AlertBanner message={error} />
        {rows.map((row, idx) => (
          <div key={row.id} className="bg-slate-50 rounded-xl border border-slate-200 p-6 space-y-5">
            <div className="flex items-center justify-between">
              <p className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">Discount Rule {idx + 1}</p>
              {rows.length > 1 && (
                <button onClick={() => removeRow(idx)} className="p-1.5 rounded-lg text-rose-400 hover:text-rose-600 hover:bg-rose-50 transition-all" aria-label="Remove rule"><FiTrash2 size={13} /></button>
              )}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <FormField label="Brand" required>
                <CustomSelect value={row.brand_name} onChange={(e) => handleBrandChange(idx, e.target.value)} options={allBrands.map((b) => b.brand_name)} placeholder="Select Brand" />
              </FormField>
              <FormField label="Model" required>
                <CustomSelect value={row.model_name} onChange={(e) => updateRow(idx, { model_name: e.target.value })} options={row.brand_name ? brandToModels[row.brand_name] || [] : []} placeholder="Select Model" disabled={!row.brand_name} />
              </FormField>
              <FormField label="Discount" required>
                <div className="flex items-center gap-2">
                  <input type="number" min="0" max={row.is_percentage ? 100 : undefined} value={row.discount_value}
                    onChange={(e) => { let v = e.target.value; if (row.is_percentage && Number(v) > 100) v = "100"; updateRow(idx, { discount_value: v }); }}
                  ...
                  <DiscountToggle isPercentage={row.is_percentage} onChange={(val) => updateRow(idx, { is_percentage: val })} />
                </div>
              </FormField>
              <FormField label="Products">
                <ProductMultiSelect
                  products={(productsByBrand[row.brand_name] || []).filter((p) => row.model_name ? p.model === row.model_name || p.model_name === row.model_name : true)}
                  selected={row.product_ids} onChange={(ids) => updateRow(idx, { product_ids: ids })} />
              </FormField>
            </div>
            <FormField label="Description (Optional)">
              <div>
                <textarea rows={2} maxLength={200} value={row.description} onChange={(e) => updateRow(idx, { description: e.target.value })} placeholder="Enter description…"
                ...
                <p className="text-[10px] text-slate-400 text-right mt-1">{(row.description?.length || 0)}/200</p>
              </div>
            </FormField>
          </div>
        ))}
      ...
      <div className="px-7 py-5 border-t border-slate-100 bg-slate-50/30 flex justify-between items-center flex-shrink-0">
        <button onClick={() => setRows((prev) => [...prev, createEmptyDiscountRow()])}
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="px-5 py-2.5 border border-slate-200 rounded-xl text-slate-600 text-sm font-semibold hover:bg-slate-50 transition-all">Cancel</button>
          <button disabled={submitting} onClick={handleSubmit}
            ...
            {submitting ? <><div className="w-3.5 h-3.5 border-2 border-white/40 border-t-white rounded-full animate-spin" />Saving…</> : "Save Discounts"}
          </button>
        </div>
      </div>
    </ModalShell>
...
/* ─────────────────────────────────────────────────────────────────────
   EDIT DISCOUNT MODAL
───────────────────────────────────────────────────────────────────────*/
const EditDiscountModal = ({ discount, originalDiscount, productsByBrand, onClose, onSave, onChange }) => {
  const inputCls = "w-full border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm font-medium text-slate-800 placeholder-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 transition-all disabled:bg-slate-50 disabled:text-slate-400";
  return (
    <ModalShell maxWidth="max-w-xl">
      <ModalHeader title="Edit Discount Rule" subtitle="Update pricing configuration for this dealer" onClose={onClose} />
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <FormField label="Brand"><input value={discount.brand_name} disabled className={inputCls} /></FormField>
          <FormField label="Model"><input value={discount.model_name} disabled className={inputCls} /></FormField>
        </div>
        <FormField label="Discount">
          <div className="flex items-center gap-2">
            <input type="number" value={discount.discount_value} onChange={(e) => onChange({ discount_value: e.target.value })}
            ...
            <DiscountToggle isPercentage={discount.is_percentage} onChange={(val) => onChange({ is_percentage: val })} />
          </div>
        </FormField>
        <FormField label="Products">
          <ProductMultiSelect
            products={(productsByBrand[discount.brand_name] || []).filter((p) => discount.model_name ? p.model === discount.model_name : true)}
            selected={discount.product_ids || []} onChange={(ids) => onChange({ product_ids: ids })} />
        </FormField>
        <FormField label="Description">
          <textarea rows={3} value={discount.description && discount.description.toLowerCase() !== "null" ? discount.description : ""}
            onChange={(e) => onChange({ description: e.target.value })} placeholder="Enter description…"
            className="w-full border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm font-medium text-slate-800 placeholder-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 transition-all resize-none" />
        </FormField>
      </div>
      <div className="px-6 py-4 border-t border-slate-100 flex gap-3 flex-shrink-0">
        <button onClick={onClose} className="flex-1 py-2.5 border border-slate-200 rounded-xl text-slate-600 text-sm font-semibold hover:bg-slate-50 transition-all">Cancel</button>
        <button onClick={onSave} className="flex-1 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 active:scale-95 transition-all shadow-sm shadow-indigo-200">Update Discount</button>
      </div>
    </ModalShell>
  );
};
...
───────────────────────────────────────────────────────────────────────*/
const BrandsModelsSection = ({ dealer, allBrands, brandToModels, onAddBrand }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/50">
        ...
        <div className="flex items-center gap-3">
          {existingBrands.length > 0 && <span className="inline-flex px-2.5 py-1 rounded-full border text-[10px] font-black bg-slate-100 text-slate-600 border-slate-200 uppercase tracking-wide">{existingBrands.length} brands</span>}
          <button onClick={onAddBrand} className="inline-flex items-center gap-2 px-4 py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-xl hover:bg-indigo-700 active:scale-95 transition-all shadow-sm shadow-indigo-200">
            <FiPlus size={13} />Add Brands
          </button>
        </div>
      </div>
      <div className="p-6">
        {existingBrands.length === 0 ? (
          <div className="text-center py-12">
            <div className="p-4 bg-slate-100 rounded-2xl inline-block mb-3"><FiTag size={22} className="text-slate-400" /></div>
            <p className="text-sm font-semibold text-slate-500">No brands assigned</p>
            <p className="text-xs text-slate-400 mt-1">Use "Add Brand & Models" to assign brands to this dealer.</p>
          </div>
        ) : (
            ...
            {existingBrands.map((brand, index) => {
              ...
              return (
                <div key={index} className="relative pl-4">
                  ...
                  {models.length > 0 ? <ul className="space-y-1">{models.map((model, i) => <li key={i} className="text-xs text-slate-500 font-medium italic">{model}</li>)}</ul> : <p className="text-xs text-slate-400 italic">No models assigned</p>}
                </div>
              );
            })}
          </div>
...
/* ─────────────────────────────────────────────────────────────────────
   DISCOUNTS SECTION
───────────────────────────────────────────────────────────────────────*/
const DiscountsSection = ({ discounts, discountState, showDiscountActions, onAddDiscount, onEditDiscount, onPageChange }) => (
  <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
    <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/50">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-xl bg-indigo-50 text-indigo-600 border border-indigo-100"><FiPercent size={14} /></div>
        <div>
      ...
      {showDiscountActions && (
        <button onClick={onAddDiscount} className="inline-flex items-center gap-2 px-4 py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-xl hover:bg-indigo-700 active:scale-95 transition-all shadow-sm shadow-indigo-200">
    ...
    <div className="p-6">
      {discountState.loading ? (
        <div className="flex items-center justify-center py-16"><div className="relative w-8 h-8"><div className="absolute inset-0 border-4 border-indigo-100 rounded-full" /><div className="absolute inset-0 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin" /></div></div>
      ) : discounts.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          <div className="p-4 bg-slate-100 rounded-2xl"><FiPercent size={22} className="text-slate-400" /></div>
          <p className="text-sm font-semibold text-slate-500">No discounts configured</p>
          <p className="text-xs text-slate-400">{showDiscountActions ? "Start by adding pricing rules for this dealer." : "No pricing rules have been set up yet."}</p>
        </div>
      ) : (
        <>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/60">
                  {["Brand", "Model", "Products", "Discount", "Status", "Created", ...(showDiscountActions ? [""] : [])].map((h, i) => (
                    <th key={i} className={`px-5 py-3.5 text-[10px] font-black uppercase tracking-[0.1em] text-slate-400 whitespace-nowrap ${showDiscountActions && i === 6 ? "text-right" : "text-left"}`}>{h}</th>
                  ))}
                </tr>
              </thead>
                  ...
                  <tr key={d.dealer_discount_id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      {d.products?.length > 0 ? (
                        <div className="flex flex-wrap gap-1.5 max-w-[280px] max-h-[60px] overflow-y-auto">
                          {d.products.map((p) => {
                            ...
                            const discount = d.is_percentage ? (base * d.discount_value) / 100 : d.discount_value;
                    ...
                    {showDiscountActions && (
                      <td className="px-5 py-4 text-right">
                        <button onClick={() => onEditDiscount(d)} className="p-2 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all" aria-label="Edit discount"><FiEdit3 size={14} /></button>
                      </td>
                    )}
                  </tr>
                ))}
          ...
          {discountState.total > 0 && <MiniPagination page={discountState.page} total={discountState.total} limit={discountState.limit} onPageChange={onPageChange} />}
        </>
      )}
    </div>
  </div>
...
/* ─────────────────────────────────────────────────────────────────────
   ORDER HISTORY SECTION — isolated, self-contained
   Has its own state for filters, pagination, loading.
   Parent passes only: dealerId, canViewOrderDetails, onViewOrder
───────────────────────────────────────────────────────────────────────*/
const ORDER_HISTORY_LIMIT = 5;

const OrderHistorySection = ({ dealerId, canViewOrderDetails, onViewOrder }) => {
  /* ── Local state — fully isolated from parent ─────────────────────*/
  ...
  const hasActiveFilters = Boolean(searchQuery || selectedStatus !== "ALL" || selectedPriority !== "ALL" || startDate || endDate);

  const clearFilters = useCallback(() => {
    setSearchInput(""); setSearchQuery("");
    setSelectedStatus("ALL"); setSelectedPriority("ALL");
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      {error && (
        <div className="px-6 py-4">
          <div className="flex items-center gap-2 px-4 py-3 bg-rose-50 border border-rose-200 text-rose-700 rounded-xl text-sm font-semibold">
        ...
        {loading && orders.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-xs text-slate-400">
              {hasActiveFilters ? "Try adjusting your filters." : "Orders placed by this dealer will appear here."}
            </p>
          </div>
        ) : !error && (
          <>
            <div className="overflow-x-auto rounded-xl border border-slate-200">
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/60">
                    {["Order ID", "Date", "Items", "Priority", "Status", ...(canViewOrderDetails ? [""] : [])].map((h, i) => (
                      <th key={i} className={`px-5 py-3.5 text-[10px] font-black uppercase tracking-[0.1em] text-slate-400 whitespace-nowrap ${canViewOrderDetails && i === 5 ? "text-right" : "text-left"}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {orders.map(({ order }) => (
                    <tr key={order.order_number} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      {canViewOrderDetails && (
                        <td className="px-5 py-4 text-right">
                          <button onClick={() => onViewOrder(order.order_number)}
            ...
            {total > 0 && <MiniPagination page={page} total={total} limit={ORDER_HISTORY_LIMIT} onPageChange={setPage} />}
          </>
        )}
      </div>
    </div>
...
const DealerDetails = () => {
  ...
  /* ── Page-level state (dealer info, discounts, brands) ────────────*/
  const [dealer, setDealer] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  const [discounts, setDiscounts] = useState([]);
  const [discountState, setDiscountState] = useState({ loading: false, error: "", page: 1, limit: 5, total: 0 });
  const [allBrands, setAllBrands] = useState([]);
  const [productsByBrand, setProductsByBrand] = useState({});

  const [orderStats, setOrderStats] = useState({ total: 0, pending: 0, inProduction: 0, inPacking: 0, delivered: 0, cancelled: 0 });

  /* ── Modal state ──────────────────────────────────────────────────*/
  const [activeModal, setActiveModal] = useState(null);
  const [selectedDiscount, setSelectedDiscount] = useState(null);
  const [originalDiscount, setOriginalDiscount] = useState(null);
  const closeModal = useCallback(() => { setActiveModal(null); setSelectedDiscount(null); setOriginalDiscount(null); }, []);

  const brandToModels = useMemo(() => buildBrandModelMap(allBrands), [allBrands]);

  /* ── Load dealer info (no orders — orders section is self-managed) ─*/
  ...
  const loadDiscounts = useCallback(async (page = 1) => {
    try {
      ...
      setDiscountState((s) => ({ ...s, page: res.pagination?.page || page, total: res.pagination?.total || 0 }));
    } catch (err) {
      setDiscounts([]);
  ...
  const loadOrderSummary = useCallback(async () => {
    ...
    for (const { key, value } of statuses) {
      ...
      } catch (err) { console.error(`Order summary fetch failed for status "${value}":`, err); }
    }
    setOrderStats(stats);
  }, [id]);
  ...
  const handleOpenEditDiscountModal = async (discount) => {
    const clean = { ...discount, description: discount.description || "", product_ids: discount.product_ids || [] };
    setSelectedDiscount(clean); setOriginalDiscount(clean);
  ...
  const buildUpdatePayload = (current, original) => {
    ...
    if (Number(current.discount_value) !== Number(original.discount_value)) payload.discount_value = Number(current.discount_value);
    if (Boolean(current.is_percentage) !== Boolean(original.is_percentage)) payload.is_percentage = Boolean(current.is_percentage);
    if ((current.description || "") !== (original.description || "")) payload.description = current.description || "";
    if (JSON.stringify(current.product_ids || []) !== JSON.stringify(original.product_ids || [])) payload.product_ids = current.product_ids || [];
    return payload;
  };
  ...
  const handleUpdateDiscount = async () => {
    try {
      ...
      if (Object.keys(payload).length === 1) { Swal.fire({ icon: "info", title: "No Changes", text: "No changes detected." }); return; }
    ...
    return (
      <div className="min-h-screen bg-slate-50/60 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <button onClick={() => navigate("/dealers")} className="px-4 py-2 bg-rose-600 text-white text-sm font-bold rounded-lg hover:bg-rose-700 transition-all">Back to Dealers</button>
        </div>
      </div>
    );
  if (!dealer)
  ...
  return (
    <div className="min-h-screen bg-slate-50/60 p-4 sm:p-6 lg:p-8">
      <div className="max-w-screen-xl mx-auto space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3.5">
            <button onClick={() => navigate("/dealers")} className="p-2 rounded-xl border border-slate-200 bg-white hover:border-slate-300 hover:shadow-sm transition-all group" aria-label="Back to dealers">
          ...
          <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[10px] font-black uppercase tracking-wide ${dealerIsActive ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-rose-50 text-rose-700 border-rose-200"}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${dealerIsActive ? "bg-emerald-500" : "bg-rose-500"}`} />
        ...
        {/* Brands */}
        <BrandsModelsSection dealer={dealer} allBrands={allBrands} brandToModels={brandToModels} onAddBrand={() => setActiveModal("addBrand")} />

        {/* Discounts */}
        <DiscountsSection
          discounts={discounts} discountState={discountState} showDiscountActions={showDiscountActions}
        ...
        {/* Order History — self-contained, no parent re-render */}
        <OrderHistorySection
          dealerId={id}
      ...
      {activeModal === "addBrand" && (
        <AddBrandModelsModal dealerId={id} existingBrands={dealer.brand || []} onClose={closeModal}
          onSuccess={() => { loadDealerData(); loadBrands(); }} />
      )}
      {showDiscountActions && activeModal === "addDiscount" && (
        <AddDiscountsModal dealerId={id} allBrands={allBrands} brandToModels={brandToModels} onClose={closeModal}
          onSuccess={() => loadDiscounts(discountState.page)} />
      )}
      {showDiscountActions && activeModal === "editDiscount" && selectedDiscount && (
        <EditDiscountModal
          discount={selectedDiscount} originalDiscount={originalDiscount}
          productsByBrand={productsByBrand} onClose={closeModal} onSave={handleUpdateDiscount}
