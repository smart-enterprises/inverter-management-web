...
/** Roles that are allowed to see the password column at all */
const PASSWORD_COLUMN_ROLES = new Set([
  ROLES.SUPER_ADMIN,
  ROLES.ADMIN,
  ROLES.MANAGER,
]);
...
const PASSWORD_SHIELD_MAP = {
  [ROLES.SUPER_ADMIN]: new Set(),                                       // sees everyone
  [ROLES.ADMIN]: new Set([ROLES.SUPER_ADMIN]),                    // blocked: super_admin
  [ROLES.MANAGER]: new Set([ROLES.SUPER_ADMIN, ROLES.ADMIN]),       // blocked: super_admin, admin
};
...
/**
 * Determines whether `viewerRole` is permitted to see the password
 * of a user with `targetRole`.
 *
 * For roles with no entry in PASSWORD_SHIELD_MAP (i.e. non-privileged
 * roles), visibility falls back to an identity check: they may only
 * see their own password.
 *
 * @param {string} viewerRole       - Role of the currently logged-in user
 * @param {string} targetRole       - Role of the user whose password is in question
 * @param {string} viewerEmployeeId - Employee ID of the currently logged-in user
 * @param {string} targetEmployeeId - Employee ID of the user whose password is in question
 * @returns {boolean}
 */
export const canViewPassword = (
  viewerRole,
  targetRole,
  viewerEmployeeId,
  targetEmployeeId
...
// ── Password Cell ────────────────────────────────────────────
/**
 * Renders the password cell for a given employee row.
 *
 * Visibility is determined entirely by `canViewPassword` — no
 * role logic is embedded in the JSX. The component is purely
 * presentational: it receives a boolean `isAllowed` prop so it
 * can be tested independently of auth context.
 */
const PasswordCell = ({
  employee,
  isAllowed,          // derived by parent via canViewPassword()
  isRevealed,
  onToggleReveal,
}) => {
  // Viewer does not have permission for this target's password
  if (!isAllowed) {
    return (
      <span
...
const User = () => {
  ...
  const canCreateUser = useMemo(
    () => [ROLES.SUPER_ADMIN, ROLES.ADMIN].includes(viewerRole),
    [viewerRole]
  );

  const canUpdateUser = useMemo(
    () => [ROLES.SUPER_ADMIN, ROLES.ADMIN, ROLES.MANAGER].includes(viewerRole),
    [viewerRole]
  );
  ...
  /** Whether the current user's role even allows seeing the password column */
  const showPasswordColumn = useMemo(
    () => PASSWORD_COLUMN_ROLES.has(viewerRole),
    [viewerRole]
  );
  ...
  /**
   * Tracks which employee IDs have their passwords currently revealed.
   * Reset on every fresh data load to prevent stale reveals.
   */
  ...
  const loadUsers = useCallback(async () => {
    try {
      ...
      const res = await fetchUsers({
        ...
        // Only request password data if the viewer's role permits it
        includePassword: showPasswordColumn && includePassword,
        includeDealers: false,
      });
      if (!res?.success) throw new Error(res.message);

      // Reset reveals whenever new data arrives
  ...
  const handleUpdate = async () => {
    try {
      ...
      const UPDATABLE_FIELDS = [
        ...
        "role", "status", "shop_name", "district", "town", "address",
      ];
      ...
      UPDATABLE_FIELDS.forEach((field) => {
        ...
        if (nv !== ov) {
          payload[field] = field === "employee_phone" ? Number(nv) : nv;
        }
      });
  ...
  const tableHeaders = useMemo(() => [
    "User", "Email", "Phone", "Role", "Status", "Created",
          ...
          <div className="px-5 py-4 border-b border-slate-100 bg-slate-50/40">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
              ...
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                {/* Only show password toggle to privileged roles */}
                {showPasswordColumn && (
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                  ...
                  users.map((u) => {
                    /**
                     * Compute password visibility for this row here — outside JSX —
                     * so PasswordCell stays a pure presentational component.
                     */
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Password — conditionally rendered column */}
                        {includePassword && showPasswordColumn && (
                          <td className="px-5 py-4">
                            <PasswordCell
                              employee={u}
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-3">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {ROLE_TABS.filter((r) => r !== "ALL").map((role) => (
