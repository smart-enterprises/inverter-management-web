...
import {
  ...
  FiX, FiTrash2, FiPlus, FiUsers, FiFilter,
} from "react-icons/fi";
...
/** Roles that can see the password column at all */
...
/* ─────────────────────────────────────────────────────────────────────
   RBAC — which roles each viewer can SEE (tabs) and CREATE
   MANAGER can VIEW manager-level but NOT create new managers.
───────────────────────────────────────────────────────────────────────*/
...
const CREATABLE_ROLES_BY_VIEWER = {
  ...
  [ROLES.MANAGER]: [
    // MANAGER cannot create new MANAGER accounts — only sub-roles
    ROLES.SALESMAN, ROLES.PRODUCTION, ROLES.PACKING, ROLES.ACCOUNTS, ROLES.DELIVERY,
  ],
};

/**
 * Returns the roles the current viewer is allowed to see in tabs + filter.
 * Falls back to an empty array for any role not in the map.
 */
const getViewableRoles = (viewerRole) =>
  VIEWABLE_ROLES_BY_VIEWER[(viewerRole || "").toUpperCase()] ?? [];

/**
 * Returns the roles the current viewer is allowed to assign when creating.
 */
  ...
  return (
    <div className="flex items-center justify-between px-5 py-4 border-t border-slate-100">
      ...
      <div className="flex items-center gap-1.5 ml-auto">
        <button type="button" onClick={() => handlePageChange(page - 1)} disabled={page === 1} className="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 text-slate-400 hover:bg-slate-50 hover:border-slate-300 transition-all disabled:opacity-40 disabled:cursor-not-allowed">
          <FiChevronLeft size={13} />
        </button>
        {generatePages().map((p, i) =>
          p === "..." ? <span key={`e-${i}`} className="px-1.5 text-slate-300 text-xs">…</span> : (
        ...
        <button type="button" onClick={() => handlePageChange(page + 1)} disabled={page === totalPages} className="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 text-slate-400 hover:bg-slate-50 hover:border-slate-300 transition-all disabled:opacity-40 disabled:cursor-not-allowed">
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wide border ${styleMap[key] || "bg-slate-50 text-slate-600 border-slate-200"}`}>
      {status}
    </span>
  );
};
...
const PasswordCell = ({ employee, isAllowed, isRevealed, onToggleReveal }) => {
  if (!isAllowed)
    return <span className="text-slate-300 text-xs font-medium select-none" title="Access restricted" aria-label="Password access restricted">••••••••</span>;
...
const UserActions = React.memo(({ user: u, onView, onEdit, onDelete }) => {
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      <button type="button" onClick={onView} className="p-2 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all" aria-label="View user"><FiEye size={14} /></button>
      <button type="button" onClick={onEdit} className="p-2 rounded-lg text-slate-400 hover:text-sky-600 hover:bg-sky-50 transition-all" aria-label="Edit user"><FiEdit2 size={14} /></button>
      <button type="button" onClick={onDelete} className="p-2 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-all" aria-label="Delete user"><FiTrash2 size={14} /></button>
    </div>
  );
});
...
const User = () => {
  ...
  // Tabs: "ALL" + roles the viewer is allowed to see
  ...
  /* ── Guard: if selected role tab is no longer viewable, reset ─────*/
  useEffect(() => {
  ...
  const handleCreate = async () => {
    try {
      ...
      // RBAC guard: ensure selected role is in creatableRoles
      ...
      const errors = validateEmployeeFields({
        employee_name: formData.employee_name, employee_email: formData.employee_email,
        employee_phone: formData.employee_phone, password: formData.password,
        role: formData.role, isUpdate: false, allowedRoles: creatableRoles,
      });
  ...
  const handleUpdate = async () => {
    try {
      ...
      setLoading(true);
      const UPDATABLE_FIELDS = ["employee_name", "employee_email", "employee_phone", "role", "status", "shop_name", "district", "town", "address"];
      const payload = {};
  ...
  return (
    <div className="min-h-screen bg-slate-50/60 p-4 sm:p-6 lg:p-8">
      <div className="max-w-screen-2xl mx-auto space-y-5">
        ...
        {/* ── Role Tabs — only roles the viewer can see ───────────── */}
          ...
          <div className="flex items-center gap-2 min-w-max sm:flex-wrap sm:min-w-0">
            {roleTabs.map((role) => {
              ...
              return (
                ...
                >
                  {role !== "ALL" && !isActive && <span className={`w-2 h-2 rounded-full flex-shrink-0 ${cfg?.dot || "bg-slate-400"}`} />}
                  {isActive && role !== "ALL" && <span className="w-2 h-2 rounded-full bg-white/70 flex-shrink-0" />}
                  {role === "ALL" ? "All" : getRoleLabel(role)}
                  {isActive && <span className="absolute inset-0 rounded-xl ring-2 ring-white/30 pointer-events-none" />}
                </button>
              );
            })}
          </div>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50">
                  {tableHeaders.map((h, i) => (
                    <th key={i} className={`px-5 py-3.5 text-[10px] font-black uppercase tracking-[0.1em] text-slate-400 whitespace-nowrap ${i === tableHeaders.length - 1 ? "text-right" : "text-left"}`}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                  ...
                  users.map((u) => {
                    const isPasswordAllowed = canViewPassword(viewerRole, u.role, viewerEmployeeId, u.employee_id);
                    return (
                      <tr key={u.employee_id} className="hover:bg-slate-50/60 transition-colors duration-100">
                        ...
                        {includePassword && showPasswordColumn && (
                          <td className="px-5 py-4">
                            <PasswordCell employee={u} isAllowed={isPasswordAllowed} isRevealed={revealedPasswords.has(u.employee_id)} onToggleReveal={handleToggleReveal} />
                          </td>
                        )}
                        <td className="px-5 py-4">
                          <UserActions
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4 sm:p-6">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl border border-slate-200 flex flex-col" style={{ maxHeight: "90vh" }}>
              <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/50 flex-shrink-0">
                ...
                <button type="button" onClick={() => setIsModalOpen(false)} className="p-2 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-all"><FiX size={16} /></button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-3">
                ...
                {/* Role dropdown — ONLY shows roles the viewer is allowed to create */}
                ...
                <textarea
                  rows="3" placeholder="Address" value={formData.address || ""}
              ...
              <div className="px-6 py-4 border-t border-slate-100 flex-shrink-0">
                <button type="button" onClick={handleCreate} disabled={loading} className="w-full py-3 rounded-xl bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-700 active:scale-95 transition-all disabled:opacity-60 shadow-sm shadow-indigo-200">
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4 sm:p-6">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl border border-slate-200 flex flex-col" style={{ maxHeight: "90vh" }}>
              <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/50 flex-shrink-0">
                ...
                <button type="button" onClick={() => setEditingUser(null)} className="p-2 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-all"><FiX size={16} /></button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-3">
                ...
                {/* Role dropdown in edit — viewers can only assign roles they can create */}
                <select
                  value={formData.role || ""}
                  onChange={(e) => setFormData({ ...formData, role: e.target.value })}
                ...
                >
                  {/* Include current role even if viewer can't normally create it (to preserve existing data) */}
                  {[...new Set([...(formData.role ? [formData.role] : []), ...creatableRoles])].map((role) => (
                    <option key={role} value={role}>{getRoleLabel(role)}</option>
                  ))}
                </select>
              ...
              <div className="px-6 py-4 border-t border-slate-100 flex gap-3 flex-shrink-0">
                <button type="button" onClick={() => setEditingUser(null)} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-sm font-semibold hover:bg-slate-50 transition-all">Cancel</button>
                <button type="button" onClick={handleUpdate} className="flex-1 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-700 active:scale-95 transition-all shadow-sm shadow-indigo-200">Update User</button>
              </div>
            </div>
          </div>
        </>
