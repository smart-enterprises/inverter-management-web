...
const STATUS_STYLE = {
  PRODUCTION: "m3-tone-tertiary",
  PACKED: "m3-tone-secondary",
  INVOICE: "m3-tone-secondary",
  SHIPPED: "m3-tone-warning",
};

const QtyCell = ({ qty, status }) => {
  if (!qty) {
    return <span className="text-xs m3-on-surface-variant font-medium">—</span>;
  }
  return (
    <span
    ...
    return (
      <div className="min-h-screen m3-surface-container-low-bg flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm m3-on-surface-variant font-medium">Loading summary…</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen m3-surface-container-low-bg px-4 sm:px-6 py-8">
      <div className="max-w-screen-2xl mx-auto space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold m3-on-surface tracking-tight flex items-center gap-2">
              <MdOutlineInsights size={18} className="text-blue-500" />
              Production Summary
            </h1>
          ...
          <button
            ...
            disabled={loading}
            title="Refresh"
            className="p-2.5 rounded-xl border m3-outline-variant-border m3-surface-bg m3-on-surface-variant hover:m3-on-surface hover:m3-outline-border hover:shadow-sm transition-all disabled:opacity-50 self-start sm:self-auto"
          >
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {TRACKED_STATUSES.map((s) => (
            <div
              key={s}
              className="m3-surface-bg border m3-outline-variant-border rounded-xl px-4 py-3"
            >
              <p className="text-[10px] font-black uppercase tracking-[0.12em] m3-on-surface-variant">
                {s}
              </p>
              <p className="text-lg font-bold m3-on-surface mt-1">
                {totals.byStatus[s].toLocaleString("en-IN")}
              </p>
            </div>
          ))}
          <div className="m3-solid-primary border border-blue-600 rounded-xl px-4 py-3">
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="border-b m3-outline-variant-border m3-surface-container-low-bg">
                  ...
                  <th className="px-5 py-3.5 text-left text-[10px] font-black uppercase tracking-[0.1em] m3-on-surface-variant whitespace-nowrap">
                    Product
                  </th>
                  <th className="px-5 py-3.5 text-left text-[10px] font-black uppercase tracking-[0.1em] m3-on-surface-variant whitespace-nowrap">
                    Brand / Model
                  ...
                  {TRACKED_STATUSES.map((s) => (
                    <th
                      key={s}
                      className="px-5 py-3.5 text-center text-[10px] font-black uppercase tracking-[0.1em] m3-on-surface-variant whitespace-nowrap"
                    >
                      {s}
                    </th>
                  ))}
                  <th className="px-5 py-3.5 text-right text-[10px] font-black uppercase tracking-[0.1em] m3-on-surface-variant whitespace-nowrap">
                    Total
                  </th>
                </tr>
              </thead>
                  ...
                  <tr>
                    <td colSpan={4 + TRACKED_STATUSES.length} className="px-5 py-20 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-sm font-semibold m3-on-surface-variant">No items pending</p>
                        <p className="text-xs m3-on-surface-variant">Nothing is currently in production, packing, invoicing, or shipping.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                    ...
                    return (
                      <React.Fragment key={row.product_id}>
                        <tr
                          ...
                          className="hover:m3-surface-container-low-bg transition-colors duration-100 cursor-pointer"
                        >
                          <td className="w-10 px-2 py-4 text-center">
                            {dealers.length > 0 ? (
                              isExpanded
                                ? <MdExpandMore size={14} className="text-blue-500 inline-block" />
                                : <MdChevronRight size={14} className="m3-on-surface-variant inline-block" />
                            ) : null}
                          </td>
                          <td className="px-5 py-4">
                            <p className="font-bold m3-on-surface">
                              {capitalizeFirstLetter(row.product_name)}
                            </p>
                            <p className="text-[10px] font-mono m3-on-surface-variant mt-0.5 flex items-center gap-1.5">
                              <span>{row.product_id}</span>
                              {dealerCount > 0 && (
                                <span className="inline-flex items-center gap-0.5 m3-on-surface-variant">
                                  <MdGroup size={9} /> {dealerCount} dealer{dealerCount !== 1 ? "s" : ""}
                                </span>
                              )}
                            </p>
                          </td>
                          <td className="px-5 py-4">
                            <p className="font-semibold m3-on-surface">
                              {capitalizeFirstLetter(row.product_brand)}
                            </p>
                            <p className="text-[11px] m3-on-surface-variant mt-0.5">
                              {row.product_model}
                            </p>
                          </td>
                          {TRACKED_STATUSES.map((s) => (
                          ...
                          <td className="px-5 py-4 text-right">
                            <span className="text-sm font-bold m3-on-surface">
                                ...
                                {dealers.map((d, i) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      {/* dealer header row */}
                                      <div className="flex flex-wrap items-center justify-between gap-3 m3-surface-bg rounded-lg border border-blue-100/70 px-3.5 py-2.5 mt-2">
                                        <div className="min-w-0 flex-1">
                                          <p className="text-sm font-semibold m3-on-surface truncate">
                                            {formatName(d.dealer_name) || d.dealer_id || "—"}
                                          </p>
                                          <p className="text-[10px] m3-on-surface-variant mt-0.5 truncate">
                                        ...
                                        <div className="text-right pl-3 border-l border-blue-100 ml-1">
                                          <p className="text-[9px] font-black uppercase tracking-[0.1em] m3-on-surface-variant">Total</p>
                                          <p className="text-base font-extrabold m3-on-surface tabular-nums">
                                            ...
                                            return (
                                              ...
                                              >
                                                ...
                                                <div className="flex items-center justify-between text-[11px] py-1.5 px-2 rounded hover:bg-blue-50/60 transition-colors">
                                                  <span className="inline-flex items-center gap-1.5 font-mono m3-on-surface-variant">
                                                    <MdTag size={10} className="text-blue-400" />
                                                    {o.order_number || "—"}
                                                  </span>
                                                  <span className="font-bold m3-on-surface tabular-nums">
